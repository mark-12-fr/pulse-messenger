...
(function () {
  ...
  function connectSocket() {
    ...
    socket.on('connect_error', (err) => {
      if (err && err.message === 'unauthorized') logout();
    });
  ...
  function renderBubble(m) {
    ...
    if (m.attachmentType === 'image') {
      ...
      return `<div><div class="bubble media"><img src="${escapeHtml(m.attachmentUrl)}" data-light="image" alt="image" loading="lazy">${cap}</div>${t}</div>`;
    }
    if (m.attachmentType === 'video') {
      const cap = m.body ? `<div class="caption">${escapeHtml(m.body)}</div>` : '';
      return `<div><div class="bubble media"><video src="${escapeHtml(m.attachmentUrl)}" data-light="video" controls preload="metadata"></video>${cap}</div>${t}</div>`;
    }
    if (m.attachmentType === 'file') {
      const cap = m.body ? `<div class="caption">${escapeHtml(m.body)}</div>` : '';
      return `<div><div class="bubble"><a class="file-card" href="${escapeHtml(m.attachmentUrl)}" download="${escapeHtml(m.attachmentName || 'file')}">
